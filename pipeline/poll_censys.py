...
def poll_censys(
    ...
    # Normalised to the Monday of the supplied date so the dedup key stays week-aligned.
    ...
    if snapshot_week is None:
        if _week_override:
            try:
                ...
                logger.info(
                    f"poll_censys: SNAPSHOT_WEEK override → {snapshot_week}"
                    f" (supplied: {_week_override})"
                )
            except ValueError:
                logger.warning(
                    f"poll_censys: SNAPSHOT_WEEK={_week_override!r} is not a valid"
